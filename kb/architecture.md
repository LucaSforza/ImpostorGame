...
- `src/main.ts` owns UI state, HTML rendering, events, and the game flow. `init()` loads the local snapshot; every mutation goes through `change()`, which clones the state, saves it, and then re-renders. Setup and gameplay render through the same gameplay theme. Setup exposes a compact category summary; `Cambia categorie` opens a dedicated category screen with the 15 generated `src/assets/categories/*.webp` tiles. The screen stores either all categories, one category ID, or a non-empty category-ID array. Preset player avatars use the 16 individual `src/assets/avatars/avatar-*.webp` tiles; legacy emoji values resolve to image tiles without changing stored game rules. Character setup also accepts a local upload/camera photo, stored as a resized data URL with its original aspect ratio. Character setup supports random/manual avatar choice, score-counter editing, attempt-limit editing, and confirmed deletion with selection cleanup.
- `src/game.ts` owns the `Game` model, random word selection, role assignment, localized word/hint/category projection, attempt-limit validation, the crew win condition, and pure player-stat updates for completed games.
- `src/db.ts` defines `AppData<T>`, wraps snapshot reads and writes in IndexedDB, normalizes missing legacy player stats, and deletes snapshots that fail the current shape contract.
...
The reveal card is controlled by the module variable `revealed`, which is not part of `AppData` and is never saved. `visibilitychange`, `pagehide`, and `blur` reset it to `false`, so rendering hides the card when the app loses visibility. During the unrevealed state, the card shows character artwork and can be opened with the accessible Reveal button or by swiping upward more than 55 pixels. The local snapshot still contains the active game, including the word entry, role IDs, and score-recording flag needed to resume after a reload.
